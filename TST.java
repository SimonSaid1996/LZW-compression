...
///////////////////////////////	
    ...
    public String longestPrefixOf(String s) {
        ...
        Node<Value> x = root;
        int i = 0;
        while (x != null && i < s.length()) {//so i guess the only modification i need to do here is to add something like if find, combine the chars to a string
            char c = s.charAt(i);
            if      (c < x.c) x = x.left;
	...
	///////////////
	public StringBuilder anotherLongestPrefixOf(char c,StringBuilder temp){//how can you pass the entire pattern from the st if only returning one char
	     Node<Value> x = root;//
		 int i = 0;
		 //if(temp.length()!=0){
			 while(x != null&& i < temp.length()){
				char c1 = temp.charAt(i);
				if      (c1 < x.c) x = x.left;
				else if (c1 > x.c) x = x.right;
				else {
					i++;
					x = x.mid;
				}
	...
	public StringBuilder myLongestPrefixOf(StringBuilder s){
		if (s == null || s.length() == 0) return null;
        int length = 0;
        Node<Value> x = root;
